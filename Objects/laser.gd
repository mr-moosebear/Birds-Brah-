...
func _ready():
	...
	$ColorRect.set_size(Vector2(vp_width, LASER_HEIGHT))
	$CollisionShape2D.shape = RectangleShape2D.new()
	$CollisionShape2D.shape.size = Vector2(vp_width, LASER_HEIGHT)
	set_rect_position(Vector2(0, 0))
	self.position.y = -LASER_HEIGHT

# Called every frame. 'delta' is the elapsed time since the previous frame.
func _process(delta):
	if self.state != LaserState.Firing:
		# move the laser at a constant speed in the direction of the target
		var diff = target.position.y - (self.position.y + LASER_HEIGHT / 2)
		var dir = 1 if diff > 0 else -1
		var vel_y = dir * 200
		self.position.y += vel_y * delta
	if impossible_countdown == 0:
		self.position.y = target.position.y - LASER_HEIGHT / 2
